    ...
    def forward(self, input_img, target_img):
        ...
        if self.use_mse_threshold:
            ...
            if self.mse_clip_ratio is not None:
                ...
                mse_thresholded_l1_loss = torch.where(mse_thresholded_l1_loss > max_mse_loss, torch.tensor(0.0, device=mse_thresholded_l1_loss.device), mse_thresholded_l1_loss)
                mse_thresholded_l2_loss = torch.where(mse_thresholded_l2_loss > max_mse_loss, torch.tensor(0.0, device=mse_thresholded_l2_loss.device), mse_thresholded_l2_loss)
        else:
        ...
        if self.use_mae_threshold:
            ...
            if self.mae_clip_ratio is not None:
                ...
                mae_thresholded_l1_loss = torch.where(mae_thresholded_l1_loss > max_mae_loss, torch.tensor(0.0, device=mae_thresholded_l1_loss.device), mae_thresholded_l1_loss)
                mae_thresholded_l2_loss = torch.where(mae_thresholded_l2_loss > max_mae_loss, torch.tensor(0.0, device=mae_thresholded_l2_loss.device), mae_thresholded_l2_loss)
        else:
